...
{                       /****************ADMIN****************/
    ...
    #[Route('/coaching/addCAPI/new{idCoach}', name: 'addCAPI', methods: ['GET'])]
    public function addCAPI(\Doctrine\Persistence\ManagerRegistry $doctrine,int $idCoach, Request $request, SluggerInterface $slugger, SerializerInterface $serializer): Response {
        $course = new Cours();
        $course->setEtat(0);

        $coach= $this->managerRegistry->getRepository(Coach::class)->findOneBy((['id' => $idCoach]));
        $course->setIdCoach($coach);
    ...
    #[Route('/coachApi/{id}/courses', name: 'CoachCoursesApi')]
    public function showCoachCoursesApi($id,CoursRepository $courseRepository,SerializerInterface $normalizer)
    {
        $UserViewCourses = $courseRepository->profileCoach($id);

        $json = $normalizer->serialize($UserViewCourses,'json',['groups'=>"Cours"]);
        return new Response($json);
    }
    ...
    /**
     * @Route("/summoner/{name}", name="summonerm", requirements={"name"=".*"})
     */
    public function getSummoner(ClientInterface $httpClient, RequestFactoryInterface $requestFactory,Request $request): Response
    {
        $name = $request->query->get('name');
        $DAK = "RGAPI-5560fc8a-c89a-4ef0-b15a-8c123f52170e";
        ...
        $courseRepository = $this->getDoctrine()->getRepository(Cours::class);
        if ($rank !== null) {
            $courses = $courseRepository->findByRankAndGame($rank, 'LEAGUE OF LEGENDS');
        }else $courses = $courseRepository->findByGame(3);
    ...
    /**
     * @Route("/summonerApi/{name}", name="summoner", requirements={"name"=".*"})
     */
    public function getSummonerApi(ClientInterface $httpClient, RequestFactoryInterface $requestFactory,Request $request,SerializerInterface $normalizer): Response
    {
        ...
        $DAK = "RGAPI-15bc88be-1bd6-4b00-8eae-5120e29485c3";
        // Get summoner information
...
// Get summoner's profile icon ID
        $iconId = $summoner['profileIconId'];

// Combine iconId and league information in a new associative array
        $leagueData = array(
            'iconId' => $iconId,
            'leagueInfo' => $leagues[0]
        );

// Encode the new array to JSON and return it in the response
        return new Response(json_encode($leagueData), 200, [
